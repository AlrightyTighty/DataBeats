...
export default function Artists() {
  ...
  if (filter === "verified") {
    console.log(
      "Filtering for verified artists. Total artists:",
      filteredArtists.length
    );
    filteredArtists.forEach((m) => {
      console.log(
        `Artist ${m.MusicianName}: IsVerified = ${m.IsVerified}, isVerified = ${
          m.isVerified
        }, type: ${typeof m.IsVerified}`
      );
    });
    filteredArtists = filteredArtists.filter(
      (m) => m.IsVerified === true || m.isVerified === true
    );
  ...
  filteredArtists.sort((a, b) => {
    if (sortBy === "followers") {
      ...
      const monthlyA = Number(
        a.MonthlyListenerCount ?? a.monthlyListenerCount ?? 0
      );
      const monthlyB = Number(
        b.MonthlyListenerCount ?? b.monthlyListenerCount ?? 0
      );
      ...
      <div className={styles.page}>
        <div className={styles.container}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: "none",
                  background: filter === "verified" ? "#2563eb" : "#e5e7eb",
                  color: filter === "verified" ? "#fff" : "#222",
                  fontWeight: 600,
                  cursor: "pointer",
                }}
              >
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: "none",
                  background: sortBy === "followers" ? "#10b981" : "#e5e7eb",
                  color: sortBy === "followers" ? "#fff" : "#222",
                  fontWeight: 600,
                  cursor: "pointer",
                }}
              >
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: "none",
                  background: sortBy === "monthly" ? "#10b981" : "#e5e7eb",
                  color: sortBy === "monthly" ? "#fff" : "#222",
                  fontWeight: 600,
                  cursor: "pointer",
                }}
              >
          ...
          <div style={{ marginBottom: 12, fontSize: 13, color: "#374151" }}>
            ...
            {filter === "verified" ? " (verified only)" : " (all)"}
            {sortBy === "followers"
              ? " - sorted by followers"
              : sortBy === "monthly"
              ? " - sorted by monthly listeners"
              : ""}
          </div>
          ...
          {loading ? (
            ...
            <p className={styles.status}>
              No artists found
              {filter === "verified" ? " (try showing all artists)" : ""}.
            </p>
          ) : (
